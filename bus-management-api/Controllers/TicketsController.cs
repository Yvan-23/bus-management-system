...
public class TicketsController : ControllerBase
{
    ...
    public TicketsController(BusManagementDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<IActionResult> Purchase([FromBody] PurchaseTicketDto dto)
    {
        ...
        if (schedule == null)
            return BadRequest(ApiResponse<TicketDto>.Fail("Schedule not found"));

        if (schedule.DepartureTime <= DateTime.UtcNow)
            return BadRequest(ApiResponse<TicketDto>.Fail("Cannot purchase ticket for past schedule"));
